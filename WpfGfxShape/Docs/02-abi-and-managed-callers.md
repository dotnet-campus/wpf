...
> 状态：**权威静态基线已完成；Release win-x64 原 DLL 已定位并完成来源归因，PE 深度字段、布局与运行兼容仍受后续门禁约束**  
...
| 原二进制定位 | **已定位 .NET 9.0.5 Release win-x64 `wpfgfx_cor3.dll`；来源、RID、file version 与 Release CodeView 路径线索已冻结** |
| 实际二进制导出 | **仍未枚举，不得把 106 或字符串命中写成真实 DLL 全部导出数** |
